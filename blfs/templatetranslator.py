...
regexps.append([re.compile('Download MD5 sum: (.*)$'), 'Somme de contrôle MD5&nbsp;: #1'])
...
# phrases récurrentes
regexps.append([re.compile('Install ([^ ]*) by running the following commands:$'), 'Installez #1 en exécutant les commandes suivantes&nbsp;:'])
...
regexps.append([re.compile('To start the (<command>.*</command>) daemon at boot, enable the previously installed systemd unit by running the following command as the (<systemitem class="username">root</systemitem>) user:$'), 'Pour démarrer le démon #1 au démarrage, activez l\'unité systemd précédemment installée en exécutant la commande suivante en tant qu\'utilisateur #2&nbsp;:'])
