...
function balises
{
	...
	if [[ $bdiff != 0 ]]
	then
		echo "balise" $1 ":" $bdiff >> $CHEMIN_BLFSFR/verif1.detail
	fi
        bautre=$bautre-$bdiff 
} 
...
> $CHEMIN_BLFSFR/verif1.lst
> $CHEMIN_BLFSFR/verif1.detail
...
#		fi
	...
	if [[ $bautre != 0 ]]
	then
           echo "autres balises :" $bautre >> $CHEMIN_BLFSFR/verif1.detail
	fi
     fi
  fi
